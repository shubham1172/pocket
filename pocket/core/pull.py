...
class Pull:
    ...
    def run(self):
        ...
        if not os.path.exists(fs.get_path_to_manifest(manifest_name)):
            os.makedirs(fs.get_path_to_manifest(manifest_name))

        with open(fs.get_path_to_manifest_file(manifest_name), 'w') as manifest_file:
            manifest_file.write(json.dumps(manifest))

        # save layers
        if not os.path.exists(fs.get_path_to_layers(manifest_name)):
            os.makedirs(fs.get_path_to_layers(manifest_name))
        ...
        for layer in layers:
            ...
            tarfile_name = os.path.join(fs.get_path_to_layers(manifest_name), layer) + '.tar'
